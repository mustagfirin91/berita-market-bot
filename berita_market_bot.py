...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ADMIN_ID is None:
        ADMIN_ID = user_id
    await update.message.reply_text("✅ Bot berita market siap!\nGunakan perintah /addberita HH:MM isi_berita untuk menambahkan berita harian.")
...
async def add_berita(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Format salah. Contoh: /addberita 08:00 Dolar menguat karena data CPI.")

async def list_berita(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not BERITA_LIST:
        await update.message.reply_text("📭 Belum ada berita yang ditambahkan.")
        return
...
async def cek_berita(context: ContextTypes.DEFAULT_TYPE):
    ...
    for b in kirim:
        try:
            await context.bot.send_message(chat_id=ADMIN_ID, text=f"\ud83d\udcc5 {b['jam'].strftime('%H:%M')} WIB\n\n\ud83d\udccc {b['teks']}\n\n\u2014 by @FxNewsProBot")
        except Exception as e:
